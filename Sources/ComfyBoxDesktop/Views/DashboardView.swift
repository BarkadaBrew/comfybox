// DashboardView.swift — one consolidated quickview: connection/model/queue
// status, recent renders, external-client endpoints, littleroundbox +
// get_server_health, trends, render activity + heatmap, host memory,
// watched services, and the event timeline.
//
// Formerly three separate tabs (Dashboard, Health, Server) with heavily
// overlapping content (connection status, active model, queue/render stat
// tiles, and two nearly-identical activity heatmaps all appeared 2-3x
// across them). Merged into one page per-request; Queue stays a separate
// tab since it's management tooling (pause/resume/reorder/cancel), not a
// status quickview.

import SwiftUI
import Charts

/// Trailing window for activity stats, trends, and uptime figures.
...
/// Asset kinds broken out in the Activity section. "voice" appears once
/// voice memos are tracked.
enum DashboardAssetType: String, CaseIterable, Identifiable {
...
struct DashboardView: View {
    ...
    // Render-activity data (from the DAM), broken out by asset kind.
    @State private var timestampsByKind: [String: [Date]] = [:]
    @State private var range: HealthRange = .month
    @State private var heatmapType: DashboardAssetType = .all
    ...
    private func loadActivity() async {
        ...
        timestampsByKind = (try? await store.assetCreationTimestampsByKind()) ?? [:]
    }
    ...
    private func renderProgress(_ pct: Double) -> some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack {
                Text(engine.queueInfo?.currentJobId.map { "Job \($0)" } ?? "Current render")
    ...
    // MARK: - Render activity + heatmap (merged: type filter + range filter)

    /// Timestamps for a type within the selected range.
    private func timestamps(for type: DashboardAssetType, range: HealthRange) -> [Date] {
    ...
    /// Based on ALL timestamps (not the type filter) so the calendar window
    /// doesn't jump around as you switch between Images/Videos/etc.
    private var heatmapDayCount: Int {
        if let days = range.days { return days }
        let all = timestampsByKind.values.flatMap { $0 }
        guard let first = all.min() else { return 30 }
    ...
    private var activitySection: some View {
        let filtered = timestamps(for: heatmapType, range: range)
        return VStack(alignment: .leading, spacing: 8) {
            HStack {
                sectionTitle("Activity")
                Spacer()
                Picker("", selection: $heatmapType) {
                    ForEach(DashboardAssetType.allCases) { Label($0.rawValue, systemImage: $0.symbol).tag($0) }
                }
            ...
            let summary = ActivityStats.summarize(timestamps: filtered)
            LazyVGrid(columns: [GridItem(.adaptive(minimum: 130), spacing: 12)], spacing: 12) {
                StatTile(title: "Renders", value: "\(summary.totalCount)", systemImage: heatmapType.symbol)
            ...
            if filtered.isEmpty {
                ...
                ActivityHeatmapView(
                    days: ActivityStats.dayCounts(timestamps: filtered, days: heatmapDayCount)
                )
                .padding(12)
                .background(.quaternary.opacity(0.4), in: RoundedRectangle(cornerRadius: 10))
            }
