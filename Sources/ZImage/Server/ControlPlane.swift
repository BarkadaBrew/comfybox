...
enum ControlPlaneClassifier {
  static func isSyncServable(method: String, path: String) -> Bool {
    ...
    // `Task {}` — so a saturated cooperative pool (what a blocking synchronous
    // GPU render produces) still delayed the very fields the Desktop
    // queue/progress UI polls. Every input the route reads is lock-based or a
    // plain value: `LiveHealthState.read()`, `VideoJobTracker.activeJobCount`,
    // `ReplicateVideoProxy.activeJobCount`, `LocalVideoReadinessMonitor.current()`
    // and the immutable configuration. Nothing awaits, so the WHOLE payload is
    // servable here — no subset route was needed. `/api/health` is deliberately
    // NOT matched: only the bridge strips that prefix, and the bridge never
    // claims `/health`.
    case ("GET", "/health"),
         ("GET", "/v1/queue"),
