...
| WP11b (Phase 2, OPEN) | **A continuation chunk conditions poorly on audio at any length** (1.10-1.31 vs 1.7-3.05 for a first chunk) — the main obstacle to a lip-synced multi-chunk monologue. **Leading hypothesis, and it explains BOTH observations:** `CompiledChunk.bodyWithCarryOver` (DirectorTypes.swift:634) pins the carry-over as a frame-0 keyframe at **strength 1.0**. That frame is a GENERATED frame holding a mouth caught mid-word, so full-strength i2v competes with a2v cross-attention for the mouth; the pin's influence spans a roughly FIXED number of frames, so it occupies a LARGER FRACTION of a shorter chunk. That predicts continuations get WORSE when shortened — and they did (1.31 at 273f -> 1.10 at 137f), which chunk length alone cannot explain. Test: lower the carry-over strength for audio-driven chunks only (0.6-0.8), enough for continuity without pinning the mouth, then re-measure. Cheap: one 2-chunk render. CAUTION: that same full-strength pin is what makes the seams invisible, so this trades against seam quality and must be measured on both axes | WP11a | M |
