...
| WP11a (Phase 2, DONE 2026-09-18) | Audio-driven chunks cap at 145 frames (`DirectorMath.audioDrivenChunkFrames`), gated on `drives_video` so nothing else re-chunks. A/B on one 34 s take, only the chunking changed: 3x273f = chunk0 1.03 / continuation pooled 1.31; 6x137f = chunk0 **3.05** / continuation pooled 1.10. Whole-clip response 1.20 -> 1.45, r 0.132 -> 0.195. NOTE the original rationale overgeneralised: the 1.92/1.73 measurements behind it were all FIRST chunks (single-chunk renders). Shortening transforms chunk 0 and does nothing for continuations | WP11 | S |
| WP11b (Phase 2, OPEN) | **A continuation chunk conditions poorly on audio at any length** (1.10-1.31 vs 1.7-3.05 for a first chunk) — the main obstacle to a lip-synced multi-chunk monologue. **CARRY-OVER PIN STRENGTH IS RULED OUT (tested 2026-09-18 06:21).** Same generated carry-over frame, same voice slice, same seed, ONLY the frame-0 keyframe strength changed: 1.0 -> loud/quiet 0.68; 0.7 -> 0.70. A 0.02 difference is nothing, so the full-strength pin in `bodyWithCarryOver` is NOT what suppresses audio conditioning, and lowering it would cost seam quality for no gain. Caveat: both arms scored below 1.0 with negative r on only 20 loud frames, so trust the COMPARISON, not the absolute values — a standalone i2v render is an imperfect stand-in for an in-sequence chunk. Remaining suspects, untested: the a2v cross-attention may weight the conditioning latent by its position in the chunk; the audio slice offset for continuation chunks may be subtly wrong; or the effect may be specific to starting from a GENERATED frame rather than a photograph. Next probe should instrument the a2v attention directly rather than guess again | WP11a | M |
