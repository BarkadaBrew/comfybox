...
| WP11b (Phase 2, OPEN) | **A continuation chunk conditions poorly on audio at any length** (1.10-1.31 vs 1.7-3.05 for a first chunk). This, not chunk length, is now the main obstacle to a lip-synced multi-chunk monologue. The carry-over frame is the obvious suspect — it is the only thing distinguishing those chunks. Check whether the carry-over i2v conditioning overwhelms the a2v cross-attention, and whether `identity_anchor_strength` / `color_anchor` compete with it | WP11a | M |
