...
| WP11a (Phase 2, DONE 2026-09-18) | Audio-driven chunks cap at 145 frames (`DirectorMath.audioDrivenChunkFrames`), gated on `drives_video` so nothing else re-chunks. Measured as loud/quiet mouth-motion ratio, which unlike a correlation does not depend on how much of the track is speech: 145 f = 1.92 / 1.73; 273 f = 1.03 / 1.27 / 1.30. Chunk 0's 1.03 is no response at all. The VOICE barely matters (1.73 vs 1.92) — chunk length is the lever. Cost: a driven timeline reaches the 16-chunk budget at a shorter duration, surfaced as an explicit `timeline_too_long` reason | WP11 | S |
