...
| WP11a (Phase 2, **REVERTED 2026-09-18**) | The 145-frame audio chunk ceiling is REVERTED. Its evidence was an artifact: the loud-vs-quiet mouth-motion metric counted the ONE-TIME transient of the subject raising her head from the opening keyframe as the voice starts. That transient is ~40 frames, so it is 29% of a 137-frame chunk and 15% of a 273-frame one — enough alone to produce the '1.03 -> 3.05' that justified the change. Excluding the first 48 frames collapses it (2.45 -> 1.05). The clean comparison is CONTINUATION chunks, which have no onset: 273f pooled 1.31 vs 137f pooled 1.10 — **longer scored better**. The trained 289 default stands | WP11 | S |
| WP11b (Phase 2, **OPEN — earlier root cause RETRACTED**) | Sustained lip sync is WEAK across every setup measured. The 'carry-over catches the mouth mid-word' root cause is **wrong**: the actual carry-over frame was inspected and her mouth is CLOSED and the pose neutral. The photo-vs-generated gap (2.45 vs 0.68) was largely the onset transient above — after excluding it, all three setups sit at 0.75-1.27 with only ~10 loud frames each, i.e. within noise. **What still holds:** `drives_video` works (the crossover in [[audio-driven-video-verified]] is immune to this confound — an onset appears in both arms and cannot produce a diagonal), and the audio-sigma fix (#500) is a valid same-setup A/B. **What is needed next:** a metric that excludes the opening transient by construction (measure only after the subject is settled, or use a viseme/landmark tracker rather than raw pixel difference) and enough loud frames for the numbers to mean something. Do not draw conclusions from cross-setup comparisons on the current metric | WP11 | M |
