...
| WP11b (Phase 2, **ROOT-CAUSED 2026-09-18 07:03**) | A continuation chunk conditions poorly on audio because of **what it starts from** — not chunk length, not pin strength. Isolated one variable at a time, same voice/seed, loud-vs-quiet mouth motion: photo keyframe + audio@0 = 1.73; photo keyframe + audio@**136** = **2.45** (so the continuation audio OFFSET is fine — it scores BEST of the three); **generated carry-over frame** + audio@136 = **0.68**. Pin strength ruled out separately (1.0 -> 0.68, 0.7 -> 0.70). The carry-over catches the mouth MID-WORD and that pose fights the voice for the rest of the chunk. **THE FIX IS ALREADY IN WP11's OWN SPEC AND WAS NEVER BUILT: 'pause-snapped chunk boundaries'** (see the WP11 row above and §6 ladder item 9). `DirectorMath.chunkLayout` is pure arithmetic that never sees the audio, so boundaries land wherever the frame count puts them. Snap an audio-driven boundary into a PAUSE and the carry-over catches a closed mouth — the photo-like condition that measured 2.45 | WP11a | M |
