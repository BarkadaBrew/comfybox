...
*Decision:* option (c). Add an optional `shift: Float?` request field. `nil` (the default) = today's resolution-dependent `mu`, so every existing render is unmoved. A non-nil value **is `mu`** for schedule construction (amended by Addendum A.1, implemented in WP-E12b; v2's `mu = log(shift)` is withdrawn): ComfyUI registers Krea 2 as `ModelSamplingFlux(shift=1.15)`, whose `flux_time_shift(mu=shift, t) = e^mu / (e^mu + 1/t − 1)` is the same function as `Krea2Sampling.timesteps`, so the wire's `shift` and our `mu` are one parameter and the effective linear shift is `e^shift` (≈ 3.158 at 1.15). Under the Krea 2 family every table-backed schedule (`beta`, `beta57`, and the `karras`/`exponential` bounds) is built from that `mu` on the 10 000-entry Flux table (`ZImageSchedulerConfig.modelSampling = .flux(tableSize: 10000)`), never the 1000-entry DiscreteFlow table, and `SchedulerFactory` refuses them without `mu`. The `krea2-reference` preset sets `shift: 1.15`. `RenderRecipe` records `mu`, `shift` (the effective `e^mu`), and `shift_source: "dynamic" | "explicit"`.
...
- `Krea2Pipeline.swift` — `+Krea2Sampling.schedulerConfig(shift:)`, a synthetic `ZImageSchedulerConfig` carrying the constants baked into `timesteps` (Krea 2 has no `scheduler_config.json`). **Its values are specified, not left implicit** (v1 left them undefined, which made AC-14 and D11 untestable — `flowMatchingSigmaBounds` reads `config.shift` and `numTrainTimesteps` for karras/exponential/beta, and `SigmaSchedule.flow` applies its warp only when `useDynamicShifting == true`):
  ...
  ```
  **Amended by Addendum A.1 / WP-E12b:** `schedulerConfig()` takes no `shift:` parameter — Krea 2's shift is `mu`, carried beside the config by `Krea2Sampling.ScheduleShift`, and the config gains `modelSampling: .flux(tableSize: 10000)` (ComfyUI's `ModelSamplingFlux`). `config.shift` stays `1.0` and is read only by `.flow`'s base grid. The table-backed schedules build the Flux table from `mu`: `beta`/`beta57` index it, and `flowMatchingSigmaBounds` for `karras`/`exponential` is its `(σ[0], σ[last]) = (e^mu/(e^mu + 9999), 1.0)` — `3.1575e-4` at `shift: 1.15`, the fixture's `model_samplings.flux.sigma_min` — so with `shift: nil` and `shift: 1.15` alike the bounds and the warp move together from one `mu`. Families that decode a `scheduler_config.json` stay `.discreteFlow`, `mu`-free and bit-unchanged. Pinned by AC-14 and AC-21.
...
- **Addendum (comfybox#285, 2026-09-04):** the middle step above shipped as a schema field (`ImagePreset.vae`, WP-E20) but was never wired into `POST /v1/generate {"preset": id}`'s own expansion (`PresetLoRAStack.decide`, #286) — a preset's declared `vae` sat on disk and was never applied. #285 closes it the same way `model`/`steps`/`guidance` already work: `decide` adopts `declared.vae` only when the request sent none, `expandingPreset` writes it onto `payload.vae` and sets the engine-only `presetVAEApplied` flag (mirroring `presetStackApplied`), and `Krea2VAESelector.resolve` takes a `fromPreset` bit fed from that flag so `vae_source` still reads `"preset"` rather than collapsing indistinguishably into `"payload"`. No change to the reload-in-place mechanism, the key-map, or AC-56/57/59 below — this is purely the missing wire between an existing preset field and the existing selector.
...
static func discreteFlowSigmaTable(shift: Float, numTrainTimesteps: Int) -> [Float]
  // ModelSamplingDiscreteFlow: σ[i] = shift·t/(1+(shift−1)·t), t = (i+1)/T   (Z-Image and every decoded scheduler_config.json)
static func fluxSigmaTable(shift: Float, tableSize: Int = 10000) -> [Float]
  // ModelSamplingFlux: σ[i] = e^shift/(e^shift + 1/t − 1), t = (i+1)/N        (Krea 2; `shift` here IS mu — A.1)
static func beta(numSteps: Int, sigmaTable: [Float],
                 alpha: Float = 0.6, betaParam: Float = 0.6) -> [Float]
  // ts = rint(betaPPF(1 − i/steps, α, β) · (len−1)); dedupe consecutive; append 0
```
`betaPPF` inverts the regularized incomplete beta function by bisection to machine precision (the midpoint-rule CDF v2 proposed mis-rounds indices on the 10 000-entry table) — the result is `rint`-ed to an integer index. `beta57` = `(0.5, 0.7)`. **Which table is the family's** (Addendum A.1, WP-E12b): `SchedulerFactory.sigmaTable(schedule:config:mu:)` switches on `ZImageSchedulerConfig.modelSampling` — `.discreteFlow` builds from the config's linear `shift`/`numTrainTimesteps` and never reads `mu`; `.flux(tableSize:)` (Krea 2) builds from the render's `mu` and throws `missingMu(schedule)` without it. `karras`/`exponential` take their bounds from the same table's ends. `SchedulerFactory` reads the produced count and constructs with the **actual** step count; `stepsEffective` reports it.

This WP also lands the `shift` request field (D3) and threads it into `resolveSigmas` for every model-consulting schedule — as `mu` itself (A.1), not `log(shift)`.
...
- **Order ramp with a ralston warm-up.** `rk_coefficients_beta.py:1343,1376`: while `step < order + multistep_extra_initial_steps` — and `multistep_extra_initial_steps` defaults to **1** — the sampler is swapped to `ralston_{order}s`, so `deis_3m` warms up for **4** steps (0…3), not 3 (corrected by Addendum A.1; v2 wrote `step < order`). At the published stage-2 settings (`deis_3m`, 2 steps) **both steps run `ralston_3s`** — 6 forward passes, and true DEIS coefficients never engage. `warmupSampler`/`warmupSteps` in the record exist so nobody rediscovers this.
...
| 21 | **`beta` matches ComfyUI at Krea 2's registered shift — on the `ModelSamplingFlux` table** (re-pinned by Addendum A.1, WP-E12b): `beta(6)` at `shift: 1.15` == `[1.0, 0.969095, 0.892582, 0.759584, 0.545649, 0.241540, 0.0]` and `beta57(6)` == `[1.0, 0.941558, 0.823777, 0.640931, 0.390072, 0.125063, 0.0]` to 1e-5 (`comfy_sigmas.json` `model_samplings.flux`, ComfyUI `783545f`; scipy re-run 2026-08-22 agrees to 1e-6); `beta(9)`, `beta(30)`, `beta57` match their `flux` fixtures through the factory; the dynamic case (`shift` omitted, mu 0.90625 at 1024²) indexes the same table. Built through `Krea2Sampling.resolveShift(explicit: 1.15)` → `(config, mu)` → `SchedulerFactory.resolveSigmas(.beta)`, so this criterion tests the render path. The v2 values (`σ₁ = 0.919919`) are the 1000-entry DiscreteFlow grid and survive only as the D5 before/after record. | `BetaScheduleComfyParityTests.testBetaMatchesComfyAtKrea2Shift`, `.testBeta57MatchesComfyAtKrea2Shift`, `.testKrea2FamilyBetaAndBeta57MatchFluxFixtures`, `.testDynamicMuBetaIsTheFluxGridAtResolutionMu`, `.testKrea2FamilyTableSchedulesRequireMu` |
...
| 24 | **`deis_3m` warm-up is faithful**: `maxOrder 3`, 2 steps → `warmup_sampler == "ralston_3s"`, `warmup_steps == 2`, **6** model evaluations. At 8 steps → `warmup_steps == 4` (RES4LYF `step < order + multistep_extra_initial_steps`, the latter defaulting to 1 — `rk_coefficients_beta.py:1343,1376`; corrected by Addendum A.1, v2 wrote 3) and steps 4–7 use order-3 coefficients. | `DEISMultistepSchedulerTests.testOrderRamp` |
...
**R11 — Sigma-grid divergence from ComfyUI at the model level** (D3). Our mu at 1024² is 0.906 (effective shift 2.475) vs ComfyUI's fixed mu 1.15 (effective `e^1.15` ≈ 3.158) — and until WP-E12b the engine also read the published `shift: 1.15` as a *linear* shift (mu 0.14) on the wrong table, so "parity" was unreachable by construction (A.1). This is the single most likely reason a "parity" render still will not look like the author's. *Mitigation:* the `shift` field, the reference preset stating 1.15, and `shift_source` in the record. `bong_tangent` is immune.
